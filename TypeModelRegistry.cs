...
namespace FlatBuffers
{
    ...
    {
        private static readonly TypeModelRegistry SDefault = new TypeModelRegistry();

        /// <summary>
        /// Gets the globally shared-Default instance of the TypeModelRegistry
        /// </summary>
        public static TypeModelRegistry Default => SDefault;

        private readonly Dictionary<Type, TypeModel> _typeModels = new Dictionary<Type, TypeModel>();

        private static readonly Dictionary<Type, BaseType> ClrTypeToBaseType;

        static TypeModelRegistry()
        {
            ClrTypeToBaseType = new Dictionary<Type, BaseType>
            {
        ...
        private BaseType DeduceBaseType(Type type)
        {
            ...
            {
                BaseType baseType;
                if (!ClrTypeToBaseType.TryGetValue(type, out baseType))
                {
        ...
        {
            if (_typeModels.TryGetValue(type, out var typeModel))
            {
                return typeModel;
            }
